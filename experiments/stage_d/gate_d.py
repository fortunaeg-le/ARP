...
#: Текст отказа. Вынесен константой, чтобы тест мог проверить не «упало», а
#: что человек получил объяснение и адрес актуальной проверки.
RETIRED_REFUSAL = """\
================================================================================
ГЕЙТ D ОТСТАВЛЕН И ЗАПУСКАТЬСЯ НЕ БУДЕТ.

Почему запуск запрещён, а не просто «не рекомендуется» (этап GATE-2):
  * его точка отсчёта (baseline_d.json) фиксирует 650 ложных срабатываний на
    объявленных негативах. Фактически их 16 — то есть рост ложных срабатываний
    В СОРОК РАЗ этот гейт напечатал бы ЗЕЛЁНЫМ;
  * он не проверяет ни leak_v2 по типам, ни masking A/B, ни порчу документа,
    ни границы по направлению ошибки — то есть «зелёный гейт D» никогда не
    означал «можно коммитить»;
  * без аргумента он читает ЗАКОММИЧЕННЫЙ results_d_head.json, поэтому его
    «зелено» может относиться к коду недельной давности.

Актуальная проверка — ЕДИНЫЙ гейт, шесть линий, одна точка отсчёта:
    venv/Scripts/python.exe tests/corpus/gate.py
    venv/Scripts/python.exe tests/corpus/gate.py --results <готовый дамп>

ЧТО СОХРАНЕНО. Файл и его числа НЕ удалены намеренно: baseline_d.json и
results_d_head.json — провенанс цифр этапа D, на них ссылается
docs/archive/reports/HANDOFF_STAGE_D_PRECISION.md, а функции compute_state /
evaluate остаются рабочими и покрыты историческим зеркалом tests/test_gate_d.py.
Запрещён именно ЗАПУСК как проверки — не чтение и не история.
================================================================================"""


def main():
    # ЭТАП GATE-2, решение владельца: отставной гейт обязан быть НЕСПОСОБЕН
    # выстрелить. Отказ стоит первым действием — до чтения дампа и baseline,
    # чтобы никакая часть отчёта не успела напечататься и быть принятой за
    # результат проверки.
    print(RETIRED_REFUSAL, file=sys.stderr)
    sys.exit(2)


def _main_retired_body():
    """ИСТОРИЯ: тело main() до отставки (этап GATE-2). Не вызывается ниоткуда,
    оставлено, чтобы было видно, ЧТО именно делал гейт D, когда был живым."""
