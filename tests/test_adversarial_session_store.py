...
class TestKeyDeletedBetweenSaveAndLoad:
    ...
    "непонятный" файл, неполная синхронизация). load_session должен сообщить чистую
    ошибку и НЕ пересоздавать key.bin — молчаливое пересоздание маскирует проблему и,
    что хуже, гарантирует, что все прочие ещё живые сессии в этой директории станут
    нерасшифровываемыми (их зашифровали старым, уже несуществующим ключом).
    """

    def test_load_after_key_deletion_raises_and_does_not_recreate_key(self, tmp_path):
        ...
        # Ключ на пути чтения не пересоздаётся: если бы load_session тихо создал
        # новый key.bin, все прочие сессии в директории (зашифрованные старым
        # ключом) стали бы невосстановимы без единой ошибки.
        assert not (store / "key.bin").exists(), (
