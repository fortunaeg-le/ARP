...
Карта механизмов, контракт классов, приёмка — docs/archive/reports/T4_REPORT.md. Группы:
...
class TestSuppressionMirror:
    ...
    def test_mirror_records_event_with_required_fields(self):
        ...
        docs/archive/reports/T4_REPORT.md §0). Кто именно победит пересечение, решают ОБЫЧНЫЕ
        правила _winner (длина и т.д., см. docs/archive/reports/T_ARB_REPORT.md) — здесь барьер
