...
#: ЭТАП REG. Прежде исключался только сам манифест, а докстринг обещал «и
#: результаты прогонов». Обещание не было исполнено: `results_v2_current.json`
#: (в `.gitignore`, перезаписывается КАЖДЫМ замером v2) попадал в манифест, и
#: состав манифеста зависел от того, гонял ли автор замер, — ровно это дало
#: расхождение 312/311 файлов между ветками `debt1` и `markup2` на MERGE-2.
#: Файл, которого нет в истории, не может быть предметом байтовой сверки:
#: манифест на него краснеет у любого, кто замер не гонял, и зеленеет у того,
#: кто гонял, — то есть перестаёт быть общим фактом.
#: ВАЖНО: `results_v2_pre_t2.json` и `results_v2_t2.json` сюда НЕ входят — они
#: закоммичены как эталонные снимки этапов и обязаны быть под сверкой.
SKIP_FILES = {"MANIFEST.sha256", "results_v2_current.json"}
