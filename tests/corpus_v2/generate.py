...
# Строка-фантом подобрана так, чтобы реестр форм НЕ МОГ её породить: сумма не
# круглая (values._amount_round даёт только круглые), копейки 13 (порождаются
# только 00 и 50). Иначе фантом столкнулся бы с настоящей суммой из другого
# документа, и отличить «детектор вычитал удалённый текст» от «детектор нашёл
# законную величину» стало бы невозможно — класс потерь перестал бы измеряться.
_DEL_GHOST = "1 234 567 (Один миллион двести тридцать четыре тысячи пятьсот " \
