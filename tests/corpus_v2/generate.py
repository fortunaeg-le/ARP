...
def build_doc(idx, ctype, cname, fmt, flags, seed, parties_kind, long_doc,
    ...
    # Дата документа — вхождение с осью записи. Форма с кавычками
    # («24» июля 2007 г.) — одно из значений оси, а не единственная запись, как
    # было до этапа NEG. ЭТАП DATE-ON: перестала быть типизированным негативом
    # и стала ВЕЛИЧИНОЙ (values.docdate) — решение владельца «дату включить во
    # всех наборах»; ось при этом расширена словесными формами.
