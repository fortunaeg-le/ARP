...
def neg(why, type_=None, form=None, axes=None, lookalike=False, kind=None):
    ...
    ЭТАП A3 добавил ВТОРОЙ вид типизированного негатива — ДВОЙНИК (`lookalike`).
    Разница существенная, а не косметическая. Негатив CORPUS-V2-B — это
    настоящее вхождение вида данных, у которого нечего маскировать: у него есть
    форма записи из реестра `values.py` и набор осей, и он законно входит в
    покрытие осей. Двойник — это конструкция ДРУГОГО назначения, лишь ПОХОЖАЯ
    на вид данных (штрихкод EAN-13 длиной с ОГРН, КБК длиной со счётом,
    влажность в процентах): формы в реестре у него нет и быть не может, потому
    что он вообще не принадлежит этому виду данных. Считать его в покрытии осей
    значило бы разбавить проценты по осям вхождениями, которых в реестре нет,
...
def serialize(model):
    ...
    for eid, rec in o.ents.items():
        ...
        if m.get("axes"):
            ...
            e["axes"] = m["axes"]
        # ОСЬ ИСКАЖЕНИЙ РАСПОЗНАВАНИЯ (сессия NOISE-CORPUS). Три поля,
        # появляющиеся ТОЛЬКО у порождённого шумового корпуса
        # (`generate_noise.py`); в gold_v2.json их нет ни у одного вхождения.
        #   noise — класс искажения (noise/classes/<id>.yaml);
        #   clean — ПРАВИЛЬНОЕ значение. Портится текст документа, эталон
        #           обязан знать, что там было на самом деле;
        #   pair  — ключ пары «чистое вхождение / то же самое, испорченное»:
        #           без него сравнение «до и после» пришлось бы делать по
        #           тексту, а текст у испорченного вхождения ДРУГОЙ.
        for key in ("noise", "clean", "pair"):
            if m.get(key):
                e[key] = m[key]
        ents.append(e)
