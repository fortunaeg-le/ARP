...
негативы — исторически DOCNUM/DATE) и НЕ является точкой отсчёта гейта. Слить в один
...
#: Денежная сумма: разметка v2 зовёт её MONEY, entity_types.yaml — SUM.
#: Переименование ни там, ни там НЕ делается: имя SUM уже несёт на себе
...
#:
#: ЭТАП MARKUP-V2: та же пара для номера договора. Разметка v2 (реестр осей
#: values.AXES, validate.py, тест покрытия осей) зовёт его DOCNUM — это имя
#: реестра, оно не переименовывается, иначе сравнимость форм/осей между
#: этапами порвалась бы. Код детектора называет тот же тип CONTRACT_NO
#: (entity_types.yaml, typedefs/CONTRACT_NO.yaml). Записи DOCNUM переехали
#: из negatives в entities этим же этапом (values.docnum) — измерение TP/FP
#: против CONTRACT_NO требует алиаса ровно как у MONEY/SUM.
_GOLD_ALIAS = {"MONEY": "SUM", "DOCNUM": "CONTRACT_NO"}
...
#: ЭТАП TYPES-2 добавил CONTRACT_KIND: «договор поставки» — слова, которые в
#: договоре повторяются ДЕСЯТКАМИ вхождений по построению документа, и поиск
#: значения подстрокой объявил бы утечкой каждое ЧУЖОЕ вхождение того же вида.
#: PASSPORT_ISSUER и REGISTRY_NAME сюда НЕ добавлены сознательно: имя органа
#: («ОУФМС России по Тверской области в Заволжском р-не») случайно в анонимном
#: тексте не встретится — там текстовая метрика честна.
_POSITIONAL_TYPES = ("PERCENT", "TERM", "SHARE_PCT", "TRANCHE", "CONTRACT_KIND")

#: Типизированные негативы v2 — конструкции, которые НЕ маскируются по решению
#: владельца. Маска на них — ложное срабатывание, считается отдельно.
#:
#: ЭТАП MARKUP-V2: DOCNUM здесь больше нет — владелец решил номер договора
#: маскировать, вхождения переехали в entities (values.docnum), маска на них
#: теперь TP, а не FP. Негативы CONTRACT_NO из typedefs/CONTRACT_NO.yaml
#: (invoice/license-doc/bare-clause) сюда не входили и не входят: они
#: размечены типом CONTRACT_NO напрямую, не DOCNUM.
#:
#: ЭТАП DATE-ON: DATE ушёл отсюда ТЕМ ЖЕ ПУТЁМ и по той же причине — владелец
#: решил дату маскировать во всех наборах, вхождения переехали в entities
#: (values.docdate). Кортеж стал ПУСТЫМ; он оставлен, а не удалён, потому что
#: механизм «типизированный негатив» продолжает существовать (следующий тип
#: заведут им же), и потому что состав форменной таблицы вычитает его явно.
TYPED_NEGATIVES = ()
...
#   * типизированные негативы (`TYPED_NEGATIVES` — после DATE-ON кортеж ПУСТ)
#     исключаем: у них СВОЯ таблица, где маска считается ложным срабатыванием,
#     а не полнотой. DOCNUM ушёл оттуда этапом MARKUP-V2 и попадает в форменную
#     таблицу под кодовым именем CONTRACT_NO (`_GOLD_ALIAS`); DATE ушёл этапом
#     DATE-ON и попадает в неё под своим именем.
...
def print_typed_negatives(results):
    ...
    if not by:
        print("  0 — ни одна маска не легла на типизированный негатив.")
        return
