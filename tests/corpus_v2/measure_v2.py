...
#: владельца (решение по дате документа не принято). Маска на них — ложное
#: срабатывание новых детекторов, считается отдельно.
#:
...
#   * типизированные негативы (`TYPED_NEGATIVES`, после MARKUP-V2 это только
#     DATE) исключаем: у них СВОЯ таблица, где маска считается ложным
#     срабатыванием, а не полнотой. DOCNUM оттуда ушёл этапом MARKUP-V2 и
#     потому попадает в форменную таблицу — под кодовым именем CONTRACT_NO
#     (`_GOLD_ALIAS`), как ему и положено с момента реклассификации.
