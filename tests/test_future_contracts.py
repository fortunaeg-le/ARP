...
class TestSplitEntitiesAcrossLineBreak:
    """История: счёт, разорванный переносом строки, не собирался — первые 9 цифр
    («407028106») подхватывал ЧУЖОЙ детектор KPP (9-значный шаблон), хвост из 11
    цифр утекал открытым текстом (проверено на a118c84:
    «Счёт для возврата: [KPP_1]\\n00849653120»).

    ЭТАП A6: контракт ИСПОЛНИЛСЯ — сборка через стык граничного окна
    (multispan.collect_seam_entities) собирает счёт из двух сегментов-строк;
    строгий xfail дал XPASS ровно как задумано парой «xfail + зеркало», маркер
    снят, тест остался стражем регресса сборки."""
