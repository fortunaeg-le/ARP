...
# =========================================================================== #
# Фикстура: services_0001 — .txt, features=[] (единственный класс документов корпуса
# без единого состязательного приёма). Значит утечка не может объясняться омоглифом,
# невидимым символом, разрывом или регистром — только отсутствием детектора.
...
_S3_SNILS = "793-234-941 33"
_S3_BIRTHDATE = "21.01.1960"
_S3_PASSPORT_SUBDIV = "812-400"          # код подразделения — детектора нет
_S3_PASSPORT_SERIES = "10 10 995117"     # серия+номер — детектор ЕСТЬ (зеркало)
...
class TestStage3SnilsHasNoDetector:
    ...
    def test_mirror_inn_in_same_document_is_masked(self, encrypt_doc):
        ...
        детектора SNILS нет вовсе, тип не маскируется НИГДЕ в корпусе (recall 0/228,
        BASELINE.md §«SNILS, BIRTHDATE — нет детектора»). Ближайшее честное зеркало —
        соседний числовой реквизит того же документа."""
    ...
    def test_snils_value_is_restorable_after_decrypt(self, encrypt_doc):
        ...
        run = encrypt_doc(_S3_DOC)
        restored = _decrypt(run, _anon(run))
        assert _S3_SNILS in restored, "decrypt не вернул исходный СНИЛС"
...
class TestStage3PassportSubdivisionHasNoDetector:
    ...
    def test_mirror_passport_series_and_number_in_same_document_is_masked(self, encrypt_doc):
        ...
        или метрике."""
        gold = _gold_entity(_S3_DOC, "PASSPORT", _S3_PASSPORT_SERIES)
        anon = _anon(encrypt_doc(_S3_DOC))
...
class TestStage5AddressWithoutMarkersLeaks:
    """Адрес без маркеров («город улица дом» в плотной строке) детектором не
    опознаётся, и в открытом тексте остаются ИМЕНА — город и улица."""

    @pytest.mark.xfail(strict=True, reason="этап 5: адрес без маркеров (dense_line) не детектируется — город и улица утекают")
    ...
    def test_mirror_address_with_markers_in_same_document_is_masked(self, encrypt_doc):
        ...
        различает «замаскировано» и «утекло», а xfail выше красен из-за отсутствия
        маркеров, а не из-за всегда-красной проверки.
