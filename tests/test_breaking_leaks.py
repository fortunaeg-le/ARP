...
class TestEntitySplitAcrossSegmentBoundary:
    ...
    @pytest.mark.xfail(strict=True, reason="B3: телефон, разорванный переносом абзаца, не детектится")
    ...
    @pytest.mark.xfail(strict=True, reason="B3: телефон, разорванный границей ячеек таблицы, не детектится")
    ...
    def test_org_split_across_lines_partial_leak_pin(self, tmp_path, config_path):
        """PASS(pins): организация 'ООО\\nРомашка' — распознаётся только часть,
        отличительное имя 'Ромашка' остаётся в открытом тексте. Фиксируем факт
        частичной утечки как есть."""
        anon = _anonymize_txt("Договор с ООО\nРомашка на поставку", tmp_path, config_path)
        assert "Ромашка" in anon  # отличительная часть имени организации утекла
