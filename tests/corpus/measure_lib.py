...
def _gap_is_seam(a, b, seam):
    """Целиком ли зазор [a,b) состоит из НЕМАСКИРУЕМОГО ШВА.

    `seam` — интервалы шва этого документа (`seam_spans`, координаты PT-1):
    позиции, которые (1) не принадлежат НИ ОДНОМУ сегменту, прочитанному
    системой, и (2) несут символ из `SEAM_CHARS` ('\\n' между блоками и
    строками таблицы, '\\t' между ячейками). Оба условия обязательны —
    обоснование в блоке «НЕМАСКИРУЕМЫЙ ШОВ» ниже по файлу.

    Пустой seam (или seam=None) -> False ВСЕГДА: без знания о шве прибор обязан
    вести себя ровно как прежде, а не прощать зазор «на всякий случай».
    """
    if not seam or b <= a:
        return False
    cur = a
    for s, e in seam:
        if e <= cur:
            continue
        if s > cur:
            return False          # кусок зазора не шов — прощать нечего
        cur = e
        if cur >= b:
            return True
    return cur >= b
...
def _covered_by_union(gs, ge, spans, seam=None):
    """Покрыт ли [gs,ge) объединением интервалов spans (список (s,e)).

    ЭТАП INSTR-SEAM — ЗАЗОР ШВА (починка ПРИБОРА, линия «г» masking B).
    Значение, разорванное вёрсткой, закрывается ПАРОЙ масок: половина в одном
    сегменте, половина в соседнем. Разделитель сборки PT-1 между ними не
    принадлежит ни одному сегменту, который читает система, — поставить туда
    маску нечем НИ ПРИ КАКОМ качестве детекции. Прежний прибор требовал
    НЕПРЕРЫВНОГО покрытия и браковал ОБЕ маски пары: чем больше рваных значений
    система закрывает, тем ниже линия «г» — метрика наказывала за успех
    (`CHARNORM-MASKB-B3-SEPARATOR`, `SEAMJOIN-MASKB-PAIR-SEAM`).

    ПРАВИЛО УЗКОЕ, и оно ровно то, что уже применено к линии «е» (этап REG).
    Зазор прощается, только если выполнены ТРИ условия СРАЗУ:
      1. зазор лежит МЕЖДУ двумя масками (`cur > gs`) — то есть слева от него
         маска уже есть, и справа маска начинается; обе накрывают ОДИН И ТОТ ЖЕ
         эталон (в `spans` приходят только маски, пересёкшие этот эталон, —
         см. `mc_check_bc`);
      2. каждая позиция зазора не принадлежит ни одному прочитанному сегменту;
      3. символ в каждой позиции — из `SEAM_CHARS`.
    Условия 2 и 3 проверяет `_gap_is_seam` по интервалам `seam`.

    ЧТО ПРАВИЛО НЕ ПРОЩАЕТ (и это его смысл):
      * кусок эталона, открытый ПЕРЕД первой маской или ПОСЛЕ последней, —
        условие 1 не выполнено, это недобор и утечка, как и было;
      * зазор, в котором есть хоть один НЕ-разделитель, — условие 3;
      * '\\n' или '\\t' ВНУТРИ сегмента (перенос `w:br`, табуляция `w:tab`) —
        туда маску поставить можно, значит открытый символ там настоящий:
        условие 2;
      * seam=None (вызов без знания о шве, в т.ч. все юнит-зеркала) — прежнее
        поведение слово в слово.
    """
    cur = gs
    for s, e in sorted(spans):
        if s > cur:
            if cur == gs or not _gap_is_seam(cur, s, seam):
                return False
            # зазор — шов: перешагиваем его, не считая покрытым и не считая дырой
        cur = max(cur, e)
        if cur >= ge:
            return True
    return cur >= ge
...
def mc_check_bc(mask, golds, all_masks, seam=None):
    ...
    `seam` (этап INSTR-SEAM) — интервалы НЕМАСКИРУЕМОГО ШВА этого документа
    (`seam_spans`, координаты PT-1). Зазор между двумя масками ОДНОГО эталона,
    целиком состоящий из шва, браком НЕ считается: маску туда поставить нечем.
    Правило и его границы — в докстроке `_covered_by_union`. Без seam прибор
    ...
    return {
        "scored": True,
        "b_ok": _covered_by_union(gs, ge, covering, seam=seam),
...
# =========================================================================== #
#  ЭТАП DEFAULT-GATE — ЛИНИЯ «з»: ЧТО ОТКРЫТО ПРИ НАСТРОЙКАХ ПО УМОЛЧАНИЮ      #
# =========================================================================== #
# ЗАЧЕМ ОТДЕЛЬНАЯ ЛИНИЯ, А НЕ ОТДЕЛЬНЫЙ ПРОГОН (обоснование решения).
# Гейт меряет ТОЛЬКО на полном наборе типов, а пользователь работает на наборе
# ПО УМОЛЧАНИЮ. Значит целый класс дыр невидим по построению: значение, которое
# на максимуме закрывает маска ДРУГОГО типа, при настройках по умолчанию уходит
# открытым. Класс нашёлся не гейтом и не набором тестов, а усиленным кругом на
# собранной программе — то есть случайно.
# Отдельный прогон отдельным инструментом повторил бы ровно ту ошибку, из-за
# которой класс и прожил незамеченным: то, что не гоняется в приёмке, не
# охраняет. Поэтому — линия гейта, в том же прогоне и с тем же порогом.
# Цена нулевая по времени: дорогая часть (resolve_for_masking, второй проход
# детекции по граничным окнам) считается ОДИН раз, а второе раскладывание масок
# поверх готового арбитража дешёвое (см. run_measurement.process_doc).
#
# ЧТО ИМЕННО СЧИТАЕТСЯ. Эталонная сущность типа, ВХОДЯЩЕГО в набор по умолчанию
# (пользователь ждёт маску), закрытая на МАКСИМУМЕ и открытая ПО УМОЛЧАНИЮ.
# Не считаются: типы вне набора (их открытость — работа по правилам) и
# пропуски детекции (их меряют полнота и утечка). Здесь ровно разница ДВУХ
# раскладок на ОДНОЙ И ТОЙ ЖЕ детекции — то, чего не видит ни одна другая линия.
try:                                        # src/ уже в sys.path у вызывающих
    import type_policy as _type_policy
    DEFAULT_PROFILE_GOLD_TYPES = frozenset(
        TYPE_MAP.get(t, t) for t in _type_policy.PERSONAL)
except Exception:                           # pragma: no cover
    DEFAULT_PROFILE_GOLD_TYPES = frozenset()


def uncovered_chars(gs: int, ge: int, spans) -> int:
    """Сколько символов [gs,ge) не закрыто ни одним интервалом `spans`."""
    if ge <= gs:
        return 0
    cover = bytearray(ge - gs)
    for s, e in spans:
        lo, hi = max(gs, s), min(ge, e)
        for i in range(lo, hi):
            cover[i - gs] = 1
    return (ge - gs) - sum(cover)


def default_profile_summary(results):
    """Сводка линии «з» из записей run_measurement.process_doc.

    Возвращает {"n": всего случаев, "per_type": {тип: сколько},
    "cases": [(doc_id, тип, start, end), …] отсортированные — по ним сверяется
    реестр известного долга, чтобы линия ловила не только РОСТ числа, но и
    ПОДМЕНУ одного случая другим при том же числе}."""
    per_type = {}
    cases = []
    present = False
    for r in results:
        if r.get("outcome") != "processed":
            continue
        if "default_profile_open" in r:
            # ПОЛЕ ЕСТЬ И ПУСТО — это измерение «дыр нет», а ПОЛЯ НЕТ ВОВСЕ —
            # это «не мерили». Отличать обязательно: иначе точка отсчёта,
            # снятая до появления линии, выглядела бы как чистый результат.
            present = True
        for c in r.get("default_profile_open", ()):
            per_type[c["type"]] = per_type.get(c["type"], 0) + 1
            cases.append((r["doc_id"], c["type"], c["start"], c["end"]))
    cases.sort()
    return {"n": len(cases), "per_type": per_type, "cases": cases, "present": present}


# =========================================================================== #
#  ЭТАП SINGLE-GUARD — ЛИНИЯ «и»: ЗНАЧЕНИЕ ПОД ЧУЖОЙ МАСКОЙ                    #
# =========================================================================== #
# ЗАЧЕМ ЕЩЁ ОДНА ЛИНИЯ, КОГДА ЕСТЬ «з» (долг DEFGATE-CROSSTYPE-LATENT).
# Линия «з» меряет РАЗНИЦУ двух раскладок: сколько символов эталона не закрыто
# по умолчанию сверх того, что не закрыто на максимуме. Значение, которое на
# максимуме спрятано маской ЧУЖОГО типа, а по умолчанию спрятано маской своего,
# для неё неотличимо от здорового: обе раскладки закрывают всё, разница нулевая.
# Таких значений в корпусе сотни, и защищает их РОВНО ОДНА вещь — то, что при
# выключенных типах арбитраж отдаёт спан другому кандидату. Отсюда имя: защита
# ОДИНОЧНАЯ.
#
# ЧЕМ ЭТО ОПАСНО ИМЕННО ДЛЯ ПРИБОРА. Правка арбитража, которая уводит значение
# из-под чужой маски В НИКУДА, роняет его из класса ВООБЩЕ: счёт «под чужой
# маской» при этом ПАДАЕТ, то есть выглядит улучшением, а линия «з» молчит
# (обе раскладки одинаково дырявы, разница снова нулевая). Остаётся полнота —
# а она агрегатная, с допуском в процентных пунктах, и десяток значений в ней
# растворяется. Поэтому линия «и» сверяет не число, а ПОИМЁННО состояние
# каждого случая из реестра: «было прикрыто чужим именем» -> «не прикрыто
# ничем» обязано краснеть, даже если все счётчики стали лучше.
#
# ЧТО ИМЕННО КЛАДЁТСЯ В ДАМП (run_measurement.process_doc, поле single_guard).
# Эталонная сущность типа ИЗ НАБОРА ПО УМОЛЧАНИЮ, которую на максимуме НЕ
# закрывает ни одна маска ЕЁ СОБСТВЕННОГО типа. Это шире искомого класса
# нарочно: сюда попадают и «прикрыта чужой маской» (mask_types непусто,
# uncovered_max == 0), и «не прикрыта ничем» (uncovered_max > 0) — иначе выход
# из класса вниз был бы неотличим от выхода вверх (починки типизации).


def single_guard_summary(results):
    """Сводка линии «и» по полю `single_guard` записей run_measurement.

    Возвращает:
      present   — поле в дампе ЕСТЬ (иначе «не мерили», а не «случаев нет»);
      covered   — [(doc_id, тип, start, end)] значений ПОД ЧУЖОЙ маской
                  (закрыто на максимуме чужим именем) — это и есть реестровый
                  класс одиночной защиты;
      state     — {ключ: запись} по ВСЕМ случаям класса, включая открытые:
                  по нему линия судит о судьбе реестрового случая;
      per_type  — {тип эталона: сколько} среди `covered`;
      pairs     — {(тип эталона, тип маски): сколько ЗНАЧЕНИЙ} среди `covered`;
      n_open_default / n_open_max — сколько случаев класса открыто по умолчанию
                  и на максимуме (сегодня открытые — это промахи полноты, их
                  меряют другие линии; здесь они нужны только как состояние)."""
    covered, per_type, pairs = [], {}, {}
    state = {}
    present = False
    n_open_default = n_open_max = 0
    for r in results:
        if r.get("outcome") != "processed":
            continue
        if "single_guard" in r:
            present = True
        for c in r.get("single_guard", ()):
            key = (r["doc_id"], c["type"], c["start"], c["end"])
            state[key] = c
            if c.get("uncovered_max"):
                n_open_max += 1
            if c.get("uncovered_default"):
                n_open_default += 1
            if not c.get("uncovered_max") and c.get("mask_types"):
                covered.append(key)
                per_type[c["type"]] = per_type.get(c["type"], 0) + 1
                for b in c["mask_types"]:
                    pairs[(c["type"], b)] = pairs.get((c["type"], b), 0) + 1
    covered.sort()
    return {"present": present, "covered": covered, "state": state,
            "per_type": per_type, "pairs": pairs, "n": len(covered),
            "n_open_default": n_open_default, "n_open_max": n_open_max}


def crosstype_mask_pairs(results):
    """ПОШТУЧНО ПО МАСКАМ: маска типа B легла на эталон типа A, где A входит в
    набор по умолчанию, а B — нет. Ровно тот счёт, которым мерила разведка
    (experiments/cross_pairs.py), и считается он по полям `gold_type`/`gtype`
    записей `masks` — то есть работает и на дампе, снятом до появления линии.

    Это не то же самое, что `single_guard_summary()["covered"]`, и разница
    осмысленна: здесь единица счёта — МАСКА (одно значение, разорванное
    вёрсткой на две маски чужого типа, даёт два), там — ЗНАЧЕНИЕ, и значение,
    у которого поверх чужой маски лежит ещё и своя, в класс не идёт вовсе.
    Обе цифры печатаются рядом, чтобы разницу нельзя было принять за расхождение
    ...
    for r in results:
        ...
        for m in r.get("masks", ()):
            ...
            if not a or not b or a == b:
                continue
            if a in DEFAULT_PROFILE_GOLD_TYPES and b not in DEFAULT_PROFILE_GOLD_TYPES:
                pairs[(a, b)] = pairs.get((a, b), 0) + 1
                docs.setdefault((a, b), set()).add(r["doc_id"])
                n += 1
    return {"n": n, "pairs": pairs,
            "docs": {k: len(v) for k, v in docs.items()}, "present": present}
...
# --------------------------------------------------------------------------- #
#   ЭТАП REG — НЕМАСКИРУЕМЫЙ ШОВ (починка прибора, линия «е»)                  #
# --------------------------------------------------------------------------- #
# ЧТО ЗА ШОВ. Канонический текст PT-1 склеен из блоков документа: абзацы и
# строки таблицы — через '\n', ячейки строки — через '\t' (corpus_lib._blocks_text
# и _tbl_text). Эти символы-разделители НЕ ПРИНАДЛЕЖАТ НИ ОДНОМУ сегменту,
# которые система читает и в которых расставляет маски. Замаскировать их
# невозможно ни при каком качестве детекции: маска живёт внутри сегмента.
#
# ЧТО ЭТО ЛОМАЛО. Эталонная сущность, разорванная вёрсткой (мутации cell_split /
# linebreak корпуса), проходит через шов. Обе её половины могут быть закрыты
# масками ЦЕЛИКОМ, но символ шва между ними остаётся непокрытым, и
# `uncovered_pieces` записывает его в НЕДОБОР — то есть в «часть значения
# осталась открытым текстом (утечка)». Никакой утечки при этом нет: '\n' и '\t'
# никого не идентифицируют. Из-за этого линия «е» дважды показывала рост
# недобора там, где качество не ухудшалось (этап LAYOUT: недобор сущностей +128
# при недоборе СИМВОЛОВ −718 — класс `ADDRB-NL-HOLE`).
#
# ГРАНИЦЫ ПРАВКИ (почему это не послабление метрики). Из недобора вычитаются
# ТОЛЬКО позиции, удовлетворяющие ОБОИМ условиям сразу:
#   1. позиция не принадлежит НИ ОДНОМУ сегменту, прочитанному системой, —
#      то есть маску туда поставить физически нечем;
#   2. символ в этой позиции — из закрытого набора разделителей сборки
#      (`SEAM_CHARS` = '\n', '\t').
# Одного второго условия НЕ ХВАТИЛО БЫ: '\n' и '\t' встречаются и ВНУТРИ
# сегмента (перенос `w:br`, табуляция `w:tab`) — там они маскируемы, и прощать
# их было бы настоящим послаблением. Одного первого тоже не хватило бы: если
# сегмент не удалось локализовать (`build_segment_offsets` -> misses), его текст
# выглядит как «ничей», и без проверки символа мы простили бы открытый ТЕКСТ.
# Пересечение двух условий даёт ровно символы шва и ничего больше.
#
# ЧЕГО ПРАВКА НЕ КАСАЕТСЯ: перебора (`over` считается по маскам, шов в него не
# входит), утечки (линия «б»), точности (линия «а»), порчи документа (линия «д»),
# masking A/B/C. Узость доказывается прогоном, а не этим комментарием: см.
# запись этапа REG в JOURNAL.md.

#: Разделители сборки PT-1. Закрытый набор: '\n' между блоками и строками
#: таблицы, '\t' между ячейками строки (corpus_lib). Пробел сюда НЕ входит —
#: пробел бывает значащим текстом.
SEAM_CHARS = "\n\t"


def seam_spans(segments, offsets, full_text, body_start, body_end):
    """Интервалы НЕМАСКИРУЕМОГО ШВА внутри тела документа, координаты PT-1.

    segments   — doc.segments (сегменты, которые читает система);
    offsets    — {segment_id: абсолютное смещение|None} из build_segment_offsets;
    full_text  — канонический текст PT-1 всего документа;
    body_start/body_end — границы тела в PT-1.

    Возвращает список (s, e) непересекающихся интервалов, каждый символ которых
    (а) не покрыт ни одним локализованным сегментом и (б) входит в SEAM_CHARS.
    Оба условия обязательны — обоснование в блоке выше.
    """
    covered = []
    for seg in segments:
        base = offsets.get(seg.id)
        if base is None or not seg.text:
            continue
        covered.append((base, base + len(seg.text)))
    covered.sort()
    ...
    for s, e in covered:
        ...
        cur = max(cur, e)
    if cur < body_end:
        gaps.append((cur, body_end))

    out = []
    for gs, ge in gaps:
        run = None
        for i in range(max(gs, 0), min(ge, len(full_text))):
            if full_text[i] in SEAM_CHARS:
                if run is None:
                    run = i
                ...
                run = None
        if run is not None:
            out.append((run, min(ge, len(full_text))))
    return out
...
def boundary_by_gold(golds, masks_same_type, all_mask_spans, seam=()):
    ...
    seam             — [(s,e)] интервалы НЕМАСКИРУЕМОГО ШВА (`seam_spans`),
                       ЭТАП REG. Символ шва не принадлежит ни одному сегменту,
                       маску туда поставить нечем, и считать его недобором
                       значит вменять прибору собственный артефакт сборки.
                       В `over` шов не входит и войти не может: перебор
                       считается по маскам, а маска на шве невозможна.
    ...
    for i, g in enumerate(golds):
        gs, ge = g["start"], g["end"]
        mine = assigned[i]
        under = sum(e - s for s, e in
                    uncovered_pieces(gs, ge, list(all_mask_spans) + list(seam)))
        over = 0
        for ms, me in mine:
