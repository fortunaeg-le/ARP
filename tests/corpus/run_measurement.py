...
#
# SUM добавлен этапом T-GOLD-A вместе с переклассификацией 648 сумм из негативов
# в эталонные сущности. Обоснование, что это ИМЕННО числовой тип, а не случай
# BIRTHDATE (который сюда добавлять было НЕЛЬЗЯ — точки внутри даты рвут run на
# части, метрика окна к ней неприменима): ядро суммы — цифры, разделённые
# пробелами-разрядами, ровно как у счёта, разбитого по три цифры.
#
# ЧТО МЕТРИКА ВИДИТ И ЧЕГО НЕ ВИДИТ. Десятичная запятая для v2_digit_runs —
# разделитель run-ов (склеиваются только пробел и дефис), поэтому у типичной
# суммы «893 033,00 руб.» ядро 8 цифр (89303300), а самое длинное выжившее окно
# при ПОЛНОЙ утечке — 6 (893033). Следствия, обе стороны названы явно:
#   * мягкий порог (>=6) утечку суммы ЛОВИТ — это основная линия гейта;
#   * строгий (>=8) на суммах почти не срабатывает, а утекшая целиком сумма
#     получает status "partial", а не "full" — метрика ЗАНИЖАЕТ тяжесть, но не
#     пропускает событие.
...
#
# ЭТАП T2 — PERCENT/TERM СЮДА НЕ ДОБАВЛЕНЫ, И ЭТО РЕШЕНИЕ, А НЕ ЗАБЫВЧИВОСТЬ.
# Обе метрики выше ищут ВЫЖИВШУЮ ЦИФРОВУЮ ПОДСТРОКУ значения в анонимном тексте.
# У реквизита это честно: 20 цифр счёта не встретятся в документе случайно. У
# процента и срока ядро — одна-две цифры («18» из «18%», «7» из «в течение 7
# рабочих дней»), и такая подстрока найдётся в анонимном документе десятки раз
# по совпадению: в номере пункта, в дате, внутри чужого числа. Метрика
# показывала бы утечку там, где значение закрыто маской — тот же класс ошибки,
# что уже описан у leak_v2_address («пер» ⊂ «передать»).
# Утечка этих двух типов меряется ПО ПОЗИЦИИ (сколько цифр эталонного спана не
# закрыто ни одной маской) — tests/corpus_v2/measure_v2.py, `leak_by_position`.
# В ЭТОМ корпусе вопрос теоретический: эталонных PERCENT/TERM здесь нет вовсе.
