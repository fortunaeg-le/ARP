...
(docs/archive/reports/PERF_REPORT.md §4.1), и ни один набор не совпадал ни с одним.
...
# Потолок состава. Был 25 (docs/archive/reports/PERF_REPORT.md §П.2.2 — «запас до 25
...
# (docs/archive/reports/HANDOFF_SUBSET_ITER.md §3), новый состав из 28 док. — 46.8 с, 0 крешей.
...
#
# ЭТАП T2-INN: 31 -> 33. Причина та же, что у T-GOLD-A, — РОСТ УНИВЕРСУМА.
# Разделение ИНН на два типа (`INN` = 10 цифр, организация; `INN_PER` = 12 цифр,
# человек) расщепило страты `('tct','INN',категория,trick)`: их было 13, стало
# 13 + 11 = 24 (у 11 комбинаций «категория x trick» в корпусе есть обе длины, у
# двух — только 10-значная). Универсум 251 -> 262 страты; прежний состав из 31
# документа перестал покрывать одну из них — `INN / adversarial /
# digit_grouping_dashes` (в срезе такой документ был, но с 10-значным ИНН).
# Детерминированный построитель даёт 33.
#
# ЦЕНА ЗАМЕРЕНА НА ОДНОЙ МАШИНЕ В ОДНОЙ СЕССИИ, оба состава подряд: прежние
# 31 док. — 62.1 с, новые 33 док. — 59.2 с, крешей 0 в обоих. Новый состав НЕ
# дороже старого: +2 документа тонут в разбросе повторных прогонов, а добранные
# документы оказались легче среднего (обратный случай к сессии CORPUS-V2, где
# добор был тяжелее). Полный корпус в этой же сессии — 737 с, то есть срез
# остаётся быстрее в ~12 раз, ради чего потолок и существует.
MAX_SUBSET_DOCS = 33
...
def doc_classes(d, structure):
    ...
    Универсум (docs/archive/reports/PERF_REPORT.md §3.4 «C», 191 элемент, воспроизведён
