...
# оно достижимо.
# ЭТАП T-GOLD-A: 28 -> 31. Причина — РОСТ УНИВЕРСУМА, а не медленный отбор.
# Переклассификация 648 денежных сумм из негативов в эталонные сущности типа
# SUM добавила в универсум 7 страт `('tct','SUM',категория,trick)`: одну
# canonical и шесть adversarial (`mut:case/combo/digit_spaces/homoglyph/
# invisible/linebreak` — суммы, физически испорченные мутацией). Универсум
# 247 -> 251 страт; прежний состав из 28 документов перестал покрывать четыре
# из них (combo, digit_spaces, homoglyph, invisible).
#
# ЧТО ИМЕННО НЕДОСТИЖИМО. 28 — не «неудобно», а невозможно: рандомизированный
# перебор (400 стартов жадности со случайным разрешением ничьих + прополка)
# не нашёл НИ ОДНОГО покрывающего состава короче 29 документов. Детерминированный
# построитель `build_subset_iter.py` даёт 31 и остаётся детерминированным
# намеренно: состав, найденный случайным поиском, невоспроизводим, а
# воспроизводимость состава — условие сравнимости с baseline среза. Плата за
# детерминизм — 2 документа сверх найденного минимума.
#
# ЦЕНА ЗАМЕРЕНА, а не оценена, и замерена ОБА состава НА ОДНОЙ МАШИНЕ В ОДНОЙ
# СЕССИИ: прежние 28 док. — 73.8 с, новые 31 док. — 81.7 с, рост +10.7% при
# росте состава +10.7%. Сравнивать новые 81.7 с со старой записью «46.8 с»
# (сессия CORPUS-V2) НЕЛЬЗЯ: в той сессии машина шла 1.7 с/док, в этой — 2.9
# с/док, и разница объяснялась бы скоростью машины, а не составом. Полный
# корпус в этой же сессии — 957 с, то есть срез остаётся быстрым в ~12 раз,
# ради чего потолок и существует.
#
# Ослаблением теста это не является: требование 100% покрытия страт не тронуто,
# изменилось только число, при котором оно достижимо.
MAX_SUBSET_DOCS = 31
