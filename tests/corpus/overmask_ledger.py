...
Три правила, каждое проверяется кодом, а не договорённостью:
  ...
  4. ПРАВИЛО ПРОПУСКА (этап AUDIT, 2026-08-08). Единственное основание НЕ
     вносить запись — «запись не про этот предмет»: сюда пишется движение
     ПОЛНОЙ точки отсчёта, движение среза — в `iter_baseline_ledger.json`
     (гейт сверяет этот журнал с `results_baseline.json`, и чужая запись
     покраснила бы его по чужой причине). **НИКОГДА — «иначе покраснеет
     гейт».** Краснота есть результат измерения, а не довод против записи о
     нём; журнал, из которого убирают неудобное, перестаёт быть журналом.
