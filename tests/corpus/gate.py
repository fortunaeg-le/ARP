    ...
    for r in records:
        ...
        if r.get("known_count") is not None and fact != r["known_count"]:
            print("      ⚠ число ИЗМЕНИЛОСЬ против даты решения (%d -> %d): "
                  ...
                  % (r["known_count"], fact))
        # ЭТАП REG: история цены исключения печатается ВСЕГДА, а не только при
        # расхождении. Сдвиг счёта без механизма рядом читается как тихий рост
        # цены; с механизмом — как объяснённый факт.
        for h in r.get("count_history") or ():
            print("      история: %s (%s) — %d: %s"
                  % (h.get("date", "—"), h.get("stage", "—"),
                     h.get("count", 0), h.get("why", "")))
