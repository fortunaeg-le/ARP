...
class TestPhoneSplitAcrossTxtLines:
    ...
    def test_both_halves_get_the_same_token(self, tmp_path, config_path):
        """ОБЯЗАТЕЛЬНЫЙ тест на итоговый текст: обе половины разорванной сущности
        заменяются на ОДИН И ТОТ ЖЕ токен ([PHONE_1] в конце одного сегмента и
        [PHONE_1] в начале следующего — НЕ [PHONE_1] и [PHONE_2])."""
        doc = _txt_doc(tmp_path, "Звоните +7 (495)\n123-45-67 в будни")
        anon, result = _pipeline(doc, config_path)
        assert anon.count("[PHONE_1]") == 2, f"ожидались две подстановки одного токена: {anon!r}"
        assert "[PHONE_2]" not in anon, "половины получили разные токены — недопустимо"
        phone_tokens = {e.token for e in result if e.entity_type == "PHONE"}
        assert phone_tokens == {"[PHONE_1]"}
...
class TestRequisiteSplitAcrossTableCells:
    def test_inn_split_across_adjacent_cells_shares_token(self, tmp_path, config_path, docx_factory):
        ...
        проходит чек-сумму, обе половины -> один токен [INN_1]. В итоговом тексте
        (' | ' между ячейками — только для читаемости) обе ячейки заменены."""
        ...
        assert "[INN_2]" not in anon
        inn_tokens = {e.token for e in result if e.entity_type == "INN"}
        assert inn_tokens == {"[INN_1]"}

    def test_phone_split_across_adjacent_cells_shares_token(self, tmp_path, config_path, docx_factory):
