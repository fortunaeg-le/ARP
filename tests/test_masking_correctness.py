...
def test_e2e_negative_C_account_masked_as_foreign_type(doc_id):
    ...
    зеркало на другой случай, а не что метрика сломалась»).

    ПЕРЕНОС ЗЕРКАЛА, ЭТАП E (2026-07-23). Носитель agency_0002 (ACCOUNT под KPP,
    счёт «408028104\\n45145901597», рваный переносом) ИСЦЕЛЁН мультиспан-сборщиком
    (\\n-рваные счета под якорем собираются целиком, тип верный) — тест покраснел
    вновь по своему же сценарию. Новый носитель — agency_0007: фрагмент
    «407028106» счёта под [KPP_1] (та же жадность KPP на 9-цифровом куске
    рваного счёта, чистый базовый документ, мультиспаном этапа E не покрыт —
    разрыв не по \\n внутри сегмента)."""
