...
def test_e2e_negative_C_value_masked_as_foreign_type(tmp_path, monkeypatch):
    """ОТРИЦАТЕЛЬНОЕ сквозное зеркало C: значение подхватывает ЧУЖОЙ детектор и
    прячется под НЕ своим типом. Метрика обязана назвать это нарушением C — при
    том, что по A и B такая маска может быть безупречной.

    ПЕРЕНОС ЗЕРКАЛА, ЭТАП 3. Сначала носителем был СНИЛС на agency_0003 (ловил
    KPP); этап 3 дал СНИЛС свой детектор — зеркало переехало.

    ПЕРЕНОС ЗЕРКАЛА, ЭТАП E (2026-07-23). Носитель agency_0002 (счёт, рваный
    переносом, под KPP) исцелён мультиспан-сборщиком — зеркало переехало на
    agency_0007 (фрагмент «407028106» счёта под [KPP_1]).

    ЭТАП A5 — ЗЕРКАЛО СНЯТО С КОРПУСА, УТВЕРЖДЕНИЕ ПРЕЖНЕЕ. Оба прежних
    носителя жили ОДНОЙ причиной: безъякорный KPP хватал любые 9 цифр и потому
    накрывал куски чужих реквизитов. Этап A4 дал KPP якорь-шлагбаум, и на всём
    корпусе v1 не осталось НИ ОДНОЙ маски ACCOUNT под чужим типом — переносить
    зеркало больше не на что (а искать носитель «где-нибудь ещё» значило бы
    ждать нового дефекта, чтобы проверить прибор). Поэтому носитель сделан
    СИНТЕТИЧЕСКИМ — ровно тем же приёмом, что у зеркал A и B выше: эталон
    объявляет значение счётом, система маскирует его как ИНН, метрика обязана
    увидеть C-нарушение. Проверяется то же самое, что и раньше: РАЗЛИЧАЕТ ЛИ
    метрика C тип маски и тип эталона. Утверждения не ослаблены — их два, они
    те же (нарушение найдено; тип маски не совпал с типом эталона).
    Положительная сторона — на живом корпусе, тестом ниже."""
    text = "Договор.\nИНН организации 7707083893.\nКонец.\n"
    i = text.index("7707083893")
    gold = [{"start": i, "end": i + len("7707083893"), "type": "ACCOUNT",
             "text": "7707083893", "category": "base"}]
    rec = _run_text_doc(tmp_path, monkeypatch, text, gold)
...
@pytest.mark.parametrize("doc_id", ["agency_0007"])
def test_e2e_positive_C_account_masked_under_its_own_type(doc_id):
    """ПРИШПИЛИВАЮЩИЙ тест этапа A5, обратная сторона снятого зеркала выше:
    бывший носитель нарушения C. Счёт на agency_0007 маскируется ПОД СВОИМ
    типом; вернётся безъякорная жадность KPP (или иной тип начнёт хватать куски
    счёта) — тест покраснеет."""
    ...
    acc = [m for m in rec["masks"] if m["scored"] and m["gold_type"] == "ACCOUNT"]
    assert acc, "ни одна маска не легла на эталонный счёт — детектор потерян"
    assert all(m["c_ok"] for m in acc), \
        "счёт снова маскируется под чужим типом: {}".format(
            [m["gtype"] for m in acc if not m["c_ok"]])
...
def test_contract_B_every_gold_span_is_fully_covered(contract_masks):
    """КОНТРАКТ B (жёсткий, ЗЕЛЁНЫЙ НА ЭТОМ ПОДМНОЖЕСТВЕ с этапа ADDR-B).
    ...
    ИСТОРИЯ МЕТКИ. До этапа ADDR-B тест стоял под `xfail(strict)`: на этом
    подмножестве было 15 нарушений B, ВСЕ 15 — ADDRESS (agency_0001 ×2,
    agency_0004 ×5, sale_0005 ×8), то есть ровно тот недобор границ адреса,
    который этап и чинил (аббревиатура типа улицы, «а/я», хвостовая точка
    «обл.»). Метка снята по правилу проекта: strict-xfail, ставший зелёным,
    обязан стать обычным тестом, иначе XPASS ломает прогон и достижение ничем
    не охраняется.

    ОБЛАСТЬ. Зелено ЗДЕСЬ — на шести документах _CONTRACT_DOCS, а не на всём
    корпусе: агрегатная B по 324 документам всё ещё ниже 100% (остаток —
    регистровые мутации, разрывы строкой, PASSPORT-класс), её печатает гейт.
    Этот тест охраняет достигнутое, а не объявляет задачу закрытой.
