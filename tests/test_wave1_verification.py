...
class TestP1CrossesComponent2:
    def test_boundary_pair_round_trip_through_real_docx_file(self, tmp_path, config_path, docx_factory):
        ...
        границей ЯЧЕЙКИ таблицы -> tokenize (даёт раздельные токены, P1) ->
        реальный .docx с этими токенами внутри -> file_detokenizer.detokenize_file
        (компонент 2) -> сравнить итоговый текст ячеек с исходным ПОСИМВОЛЬНО,
        включая безобидный хвост во второй ячейке.

        ЭТАП A5 — ИСПРАВЛЕН КОНТРАКТ, НЕ ОСЛАБЛЕН. Прежняя фикстура была
        'Приложение 1' | '23456789 экз.': склейка ячеек давала «Приложение
        123456789 экз.», и пару через границу порождал БЕЗЪЯКОРНЫЙ KPP, хватавший
        любые девять цифр, — то есть носителем стыка было ЛОЖНОЕ срабатывание,
        которое этап A4 снял (реальный договор без слова «КПП»: 10 ложных KPP из
        10). Тест при этом падал не на стыке, а на его отсутствии (replaced == 0).
        Утверждения теста прежние и не ослаблены: та же граница ячейки, тот же
        путь до файла и обратно, то же посимвольное равенство ОБЕИХ ячеек, то же
        `replaced >= 1`. Изменён только носитель: вместо ложного KPP — настоящий
        реквизит под своим якорем ('ИНН 7707' | '083893 экз.' склеивается в
        «ИНН 7707083893 экз.»), а безобидный хвост «экз.» остался на месте."""
        src = docx_factory("src.docx", table_rows=[["ИНН 7707", "083893 экз."]])
        doc = extract(src)
        ...
        assert table.cell(0, 0).text == "ИНН 7707"
        assert table.cell(0, 1).text == "083893 экз.", (
            "обе половины разорванного границей ячейки значения (и безобидный "
            "хвост «экз.») должны быть восстановлены ЦЕЛИКОМ через весь путь "
            "tokenize -> save_session -> file_detokenizer, а не только через "
            "detokenize() по плоской строке"
        )
