...
class TestCharNormDigitSpaces:
    """Обычный пробел ВНУТРИ цифровой группы (класс mut:digit_spaces).
    Схлопывается только запись, чья группировка НЕ разрядная, из >= 3 групп и
    >= 8 знаков — иначе разряды суммы («12 000 000») были бы склеены."""

    @pytest.mark.parametrize("base,expected", [
        ("90 14 6170 85", "9014617085"),    # паспорт: «\d{6}» разбит пробелом
        ("19 04 9493 12", "1904949312"),
    ...
    def test_non_thousands_grouping_collapsed(self, base, expected):
        ...
        _assert_map_invariant(base, norm, omap)
        assert norm == expected
        # обратная проекция обязана покрыть исходную запись ЦЕЛИКОМ
        s, e = norm_to_src(omap, 0, len(norm))
        assert (s, e) == (0, len(base))

    @pytest.mark.parametrize("base", [
        # РЕГРЕСС (поймано tests/test_regex_detector.py при первой редакции
        # прохода 3.5): хвост слова — тоже «знак цифрового вида» ('о' в «числО»,
        # 'б' в «Без»), и без требования настоящей цифры в группе пробелы вокруг
        # реквизита выбрасывались, склеивая его со словами.
        "В реестре встречается число 7707083893 без пояснений.",
        "договор о 5 000 000 рублей и 12 000 000 рублей",
        "12 000 000 руб.",   # разряды суммы
        "1 234 567,00 руб.",
        "1 000 000 000",
        "5 000",
