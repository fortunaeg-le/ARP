...
def test_default_profile_is_personal_only():
    ...
    assert set(type_policy.PERSONAL) == {
        ...
        "INN_PERSON",
        # ЭТАП MERGE-2 (санкция владельца): орган выдачи паспорта — прямые
        # ПДн, орган плюс дата сужают круг не хуже номера паспорта. Номер
        # (PASSPORT) в наборе с самого начала; набор, где номер закрыт, а
        # «кем выдан» открыт, защищал бы только на вид.
        "PASSPORT_ISSUER",
        # ЭТАП DATE-ON (решение владельца 2026-08-17 «дату включить во всех
        # наборах»): дата документа/выдачи/подписания стоит в договоре рядом с
        # ФИО и паспортом и вместе с ними сужает круг лиц. Тот же довод, что у
        # PASSPORT_ISSUER строкой выше.
        "DATE",
    }
