...
REQUIRED = ["format_split", "homoglyph", "invisible", "digit_spaces",
            "linebreak", "cell_split", "words_mismatch"]
MOST_FREQUENT = "format_split"
MIN_TYPES = 3            # приёмы обязаны лечь не на один вид данных

# Приёмы, которые по своей природе применимы к ОДНОМУ виду данных, и почему.
# Список закрытый: приём, живущий на одном виде без причины, проверяет этот
# вид, а не приём.
SINGLE_TYPE_BY_NATURE = {
    "words_mismatch":
        "цифры против прописи бывают только у суммы: у процента, срока и "
        "номера договора второй, словесной записи той же величины рядом не "
        "существует, расходиться нечему",
}
...
def test_tricks_hit_several_data_types(gold):
    ...
    thin = {t: sorted(s) for t, s in by_trick.items()
            if len(s) < MIN_TYPES and t not in SINGLE_TYPE_BY_NATURE}
    assert not thin, (
        "Приёмы легли меньше чем на %d вида данных: %s.\n"
        "Приём, живущий на одном виде, проверяет этот вид, а не приём. "
        "Исключения перечислены в SINGLE_TYPE_BY_NATURE — с обоснованием, "
        "почему приём к другим видам НЕПРИМЕНИМ, а не просто не поставлен."
        % (MIN_TYPES, thin))


def test_single_type_exemptions_are_justified():
    """Список исключений не имеет права расти молча.

    Освобождение от проверки «приём лёг на несколько видов данных» — это
    ослабление, и оно обязано быть объяснено в том же файле. Пустое или
    отписочное обоснование равносильно его отсутствию.
    """
    bad = [t for t, why in SINGLE_TYPE_BY_NATURE.items() if len(why.split()) < 10]
    assert not bad, (
        "Исключения без внятного обоснования: %s.\n"
        "Обоснование обязано объяснять, почему приём к другим видам данных "
        "НЕПРИМЕНИМ ПО ПРИРОДЕ, а не почему его туда не поставили." % bad)
