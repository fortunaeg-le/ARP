...
class TestStage2bWordLevelCase:
    ...
    def test_two_word_caps_run_is_normalized_stage_c(self, txt_factory):
        """ЭТАП C: _MIN_CAPS_RUN возвращён к 2 — двусловный заглавный прогон
        ТЕПЕРЬ нормализуется («МОСКВА ЛЕНИНА» → «Москва Ленина»), забирая
        двусловные заглавные ФИО обратно в детекцию.

        Почему уступка Stage2b-B снята безопасно: её механизм — «нормализация
        убирает СЛУЧАЙНУЮ Natasha-ORG/PER-маску и открывает адрес» — ВОИД: после
        этапов A/B Natasha-ORG и Natasha-PER выключены (случайной маски больше
        нет), а адрес теперь ЯКОРНЫЙ (не хватает заглавные прогоны как топоним).
        «МОСКВА ЛЕНИНА 47/3 КВ 67» остаётся безмаркерной дырой этапа 5 (LOC его
        не сеет), но нормализация НЕ создаёт новой утечки: он не был замаскирован
        и до правки. Проверено поштучно на подвыборке/корпусе (HANDOFF_STAGE_C)."""
        doc = extract(txt_factory("МОСКВА ЛЕНИНА 47/3 КВ 67"))
        assert _seg(doc, "l0").metadata["detection_text"] == "Москва Ленина 47/3 КВ 67"
