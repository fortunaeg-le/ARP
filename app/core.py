...
--- ЭТАП U5b (починка правки границ, по итогам docs/archive/reports/U5A_REPORT.md) -----------
...
#
# ЭТАП BUILD (2026-07-29): пересборка после T1-UI (экран выбора набора),
# T2-INN (ИНН организации/человека — два типа), ADDR-B (границы адреса),
# U4-FIX (сторож отчёта). Метка обязана нести ДАТУ: по отчёту программы
# («Сборка инструмента» в шапке, поле `tool_build`) человек должен понимать,
# какая поставка его выдала. Формат ограничен сторожом отчёта
# `report._BUILD_RE` = ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ — иначе значение
# схлопнется в «unknown» и отчёт перестанет различать сборки.
BUILD_MARK = "build-20260729"
...
TYPE_LABELS = {
    ...
    "ORG": "Организация",
    "ADDRESS": "Адрес",
    # ЭТАП T2-INN: два типа ИНН, и подпись обязана объяснять разницу человеку,
    # который не обязан знать, что 12 цифр бывают только у физлица. Число цифр
    # названо прямо — это единственный признак, по которому пользователь может
    # сам сверить наш выбор со своим документом.
    "INN": "ИНН организации (10 цифр)",
    "INN_PERSON": "ИНН человека (12 цифр)",
    "OGRN": "ОГРН/ОГРНИП",
    "KPP": "КПП",
    ...
    "SUM": "Сумма",
    # ЭТАП T2. Подписи объясняют, ЧТО именно закроется, словами договора, а не
    # кодом: юрист должен узнать в них свой текст с первого взгляда.
    "PERCENT": "Процент (ставка, неустойка)",
    "TERM": "Срок исполнения",
    # ЭТАП TYPE-FACTORY-2 — пять фабричных типов (title из typedefs).
    "REG_NUMBER": "Реестровый номер",
    "REG_ID": "Регистрационный номер в реестре",
    "CONTRACT_NO": "Номер договора",
    "SHARE_PCT": "Доля голосов / доля участия",
    "TRANCHE": "Транш (именной)",
    # ЭТАП TYPES-2 — три типа фабрики; названия взяты из `title:` их
    # файлов-описаний (tests/corpus_v2/typedefs/*.yaml).
    "PASSPORT_ISSUER": "Орган выдачи паспорта",
    "REGISTRY_NAME": "Регистрирующий орган (налоговая инспекция)",
...
def _overlapping_occurrences(session: dict, segment_id: str, start: int, end: int) -> list[tuple[dict, dict]]:
    ...
    (see docs/archive/reports/U5A_REPORT.md, вопрос 4) — недостаточно, чтобы контракт «1 маска —
...
def _validate_missed(doc, session: dict, segment_id: str, start: int, end: int,
    ...
    (docs/archive/reports/U5A_REPORT.md, вопрос 4) требует различать «ровно одна» (клиент решает
...
def _add_missed(doc, session: dict, segment_id: str, start: int, end: int,
    ...
    вручную (docs/archive/reports/U5A_REPORT.md, вопрос 3 и находка U5A-1: без этого повторная
...
def mark_reviewed(session_id: str) -> dict:
    ...
    (docs/archive/reports/U5A_REPORT.md, вопрос 6, находка U5A-6): файл разметки создаётся
