...
| (корень, сверка состояния 2026-08-19) | 3 | скриптов нет — прогон штатным `tests/corpus/gate.py` на вершине `main` (`8de62de`), сличение дампов сделано разовым сравнением в памяти | `statecheck_gate_main.log` (полный гейт на слитом `main`, 324 док., 156 с, 18 красных строк — те же, что у SEAM-JOIN; дамп совпал с закоммиченным без единого расхождения), `statecheck_gate_main_no_editable.log` (тот же гейт БЕЗ `pip install -e .` — линии «з» и «и» МОЛЧА не мерились, `STATE.md` §8а), `statecheck_pytest_main.log` (полный набор на слитом `main`: 1936 passed / 13 failed / 11 errors, все 24 — среда, разбор в `STATE.md` §8а) |
