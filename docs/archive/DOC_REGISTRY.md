...
| `metrics_full/` (METRICS-FULL) | 4 скрипта + отчёт | `measure_all.py` (запуск всех замеров подряд с хронометражем), `run_profiles.py` (ЗАМЕР ПО ЧЕТЫРЁМ НАБОРАМ ТИПОВ — того, что гейт не меряет вовсе: он всегда на «Максимуме»), `collect_metrics.py` (СБОРКА отчёта из дампов; ни одно число не пишется руками, перенесённые с прошлых дат — в одном словаре `CARRIED` с датой снимка) | `METRICS_FULL.md` + `metrics_full.json` (отчёт и его машинная копия — идут в историю: по ним следующий срез сличается автоматически), `runs.json` (хронометраж), `run_*.log` (гейт, наборы, v2, сущности, срез, набор тестов). Дампы прогонов (`results_v2.json` 8 МБ, `results_profiles.json` 1.5 МБ) в историю НЕ кладутся (`.gitignore` по месту), восстанавливаются `measure_all.py` |
