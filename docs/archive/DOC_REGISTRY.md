...
| (корень, DESKTOP-FIT / UI-FIT-2) | 8 | `desktop_fit_probe.py` (окно приложения на реальном Edge: мерит обвязку браузера по CDP и проверяет, что окно не переживает программу), `desktop_fit_layout_probe.py` (раскладка на шести размерах окна: ищет узлы, которые не влезли И не прокручиваются), `desktop_fit_built_probe.py` (то же окно на СОБРАННОМ `dist/SHIFRATOR.exe`, с аварийным убийством программы), `ui_fit2_probe.py` (обе правки UI-FIT-2 числом: высота подсказки на трёх размерах задания плюс три равной высоты и разной ширины — только так видно рост от ширины; подводка первой и последней строки под полосу AI vision; разметка мышью в обоих крайних положениях; карта риска по умолчанию; сличение ВЫЧИСЛЕННЫХ стилей плашки в описи и маски в тексте в обеих темах) | `desktop_fit_probe.log`, `desktop_fit_layout.log` (замер ДО и ПОСЛЕ правки раскладки), `desktop_fit_layout_after_uifit2.log` (шесть размеров после правок UI-FIT-2), `ui_fit2.log` |
| (корень, CHAR-NORM) | 12 | `char_norm_journal_entry.md` и `char_norm_state_head.md`/`char_norm_state_3d.md` (текст записей `JOURNAL`/`STATE`, вынесен файлом — в командной строке он не проходил) | `char_norm_probe1.log`/`char_norm_probe2.log` (разложение утечки по классам порчи и ПОИМЁННЫЙ список 200 утёкших значений ДО правки — на нём и строился этап), дамп полного гейта ДО этапа В РЕПОЗИТОРИЙ НЕ КЛАДЁТСЯ — это побайтно `git show 1f00879:tests/corpus/results_gate_current.json` (9.8 МБ дубля); в рабочем дереве этап держал его как `experiments/char_norm_gate_before.json`, `char_norm_gate_after.log` (живой прогон ПОСЛЕ), `char_norm_gate_delta.log`/`char_norm_gate_delta2.log` (оценка ТРЁХ пар одним действующим гейтом; во второй загружены реестры, первая их не грузила и потому врала по линии «з»), `char_norm_class_delta.log` (утечка по классам порчи до/после), `char_norm_residue.log` (остаток трёх классов поимённо), `char_norm_fp_delta.log`/`char_norm_overmask_delta.log`/`char_norm_maskb_delta.log` (разбор трёх красных строк дельты), `char_norm_sub_before.log`…`char_norm_sub_after4.log` (четыре прогона подвыборки), `char_norm_pytest_full*.log` (три полных набора) |
| (корень, SEAM-JOIN) | 8 | `seam_join_probe.py` (разложение утечки по приёму порчи и типу + поимённый список утекающих значений класса вёрстки), `seam_join_shape.py` (ФОРМА разрыва: внутри сегмента / через границу сегментов / соседние ячейки — этим замером и определился весь план этапа), `seam_join_ceiling.py` (ПОТОЛОК сшивки: найдётся ли значение вообще, если разрыв убрать), `seam_join_why.py` (журнал внутрисегментного ремонта с причиной каждого отказа), `seam_join_seamlog.py` (журнал стража сборки на ремонте стыка — он назвал 57 отказов PASSPORT), `seam_join_run.py` + `seam_join_diff.py` (прогон харнесса по выбранным документам и ПОИМЁННОЕ сличение двух дампов: утечка / найдено / точный спан / маски / over-mask / FP), `seam_join_maskb_probe.py` (цена немаскируемого шва в линии «г» — тот же харнесс с объединением, терпящим зазор в один символ; прибор гейта НЕ трогается) | `seam_join_probe.log`, `seam_join_shape.log`, `seam_join_ceiling.log`, `seam_join_seamlog.log`/`seam_join_seamlog2.log` (до и после правки якорей), `seam_join_gate_before_eval.log` (оценка дампа ДО этапа действующим гейтом), `seam_join_gate_after.log` (живой прогон, вскрывший линию «ж»), `seam_join_gate_after2.log` (живой прогон после правки), `seam_join_maskb.log`, `seam_join_v2_final.log` (корпус v2), `seam_join_pytest_full.log` (полный набор на финальном коде), `seam_join_accept_circle.log` (круг на собранной программе, 54/54), `seam_join_journal_entry.md`/`seam_join_findings.md` (текст записей, вынесен файлом — в командной строке он не проходил). Дампы прогонов (`sj_*.json`, 1.5-9.8 МБ) в репозиторий НЕ кладутся: дамп ДО этапа — побайтно `git show d37d53b:tests/corpus/results_gate_current.json` |
| (корень, INSTR-SEAM) | 9 | `instr_seam_narrowness.py` (ГЛАВНЫЙ прибор этапа: один прогон корпуса, КАЖДАЯ маска посчитана ДВАЖДЫ — старым прибором и новым, с поимённой проверкой узости каждого прощённого зазора), `instr_seam_dump_diff.py` (рекурсивное сличение ДВУХ дампов поле в поле — приём METRIC-FIX; принимает пути дампов аргументами), `instr_seam_maskb_by_type.py` (masking B по типам тремя колонками: точка отсчёта | старый прибор | новый, плюс masking C и счёт «утечка из одних разделителей»), `instr_seam_lines_b_e_check.py` (те же значения глазами линий «б» и «е») | `instr_seam_narrowness.log` + `instr_seam_narrowness.json` (476 КБ, ПОИМЁННО все 1049 расхождений с содержимым зазора — на нём стоит доказательство узости и его читают два прибора выше), `instr_seam_dump_diff.log`, `instr_seam_maskb_by_type.log`, `instr_seam_lines_b_e_check.log`, `instr_seam_gate_before.log` / `instr_seam_gate_after.log` (полный гейт до и после правки прибора), `instr_seam_pytest_full.log`. Дампы гейта (9.8 МБ) в репозиторий НЕ кладутся: дамп ДО правки — побайтно `git show 8de62de:tests/corpus/results_gate_current.json` |
