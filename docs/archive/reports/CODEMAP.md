...
| `regex_detector.py` | Реквизиты (ИНН организации/ИНН человека/ОГРН/тел/почта/сумма) с чек-суммами | `detect_regex(doc, cfg)`, `inn_checksum`, `inn10_checksum`/`inn12_checksum` (T2-INN: прибиты к длине), `ogrn_checksum` | `cmd_encrypt`, `tokenize` (B3-окно) |
...
| `type_policy.py` | ЭТАП T1: какие типы маскировать — четыре набора, перекрытия по типу, `~/.shifrator/settings.json` (неизвестный тип/набор игнорируются). T2-INN: `INN_PERSON` в наборе «только персональные данные», `INN` (организации) — в «данные и реквизиты» | `known_types(cfg)`, `resolve(profile, overrides, known)`, `load_settings()`, `enabled_types(cfg)`, `describe(cfg, enabled)`; `MAXIMUM is None` | `cmd_encrypt`, `app/core.current_policy` |
