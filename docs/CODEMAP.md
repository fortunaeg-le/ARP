...
| `tokenizer.py` | Разрешение пересечений; B3-проход по граничным окнам соседних сегментов; присвоение токенов; сборка анон-текста | `tokenize(doc, entities, cfg)`, `build_plain_text(doc)`; внутр.: `_resolve_overlaps`, `_trim_to_free`, `_winner`, `_detect_boundary_entities`, `_boundary_sep` | `cmd_encrypt` |
| `session_store.py` | Fernet-хранилище токен-мапы; TTL, автоочистка | `save_session`, `load_session`, `delete_session`, `purge_expired`, `default_storage_dir`; искл. `SessionNotFoundError`, `SessionExpiredError` | `cmd_encrypt/decrypt/delete`, `detokenize*` |
