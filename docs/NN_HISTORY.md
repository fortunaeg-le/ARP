...
нарицательных нового языка, `docs/DECISIONS.md`, ORG-аналог **Aprime-1**
...
Находка `T2-A` (`docs/T2_INN_REPORT.md:251-273`): 12-значный ИНН физлица,
...
(`docs/T2_INN_REPORT.md:261-262`). Следствие для набора «только персональные
данные» (тип `INN` организации выключен): такой обрывок остаётся открытым
(`docs/T2_INN_REPORT.md:267-273`).
