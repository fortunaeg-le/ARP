...
гейт/метрики:   venv/Scripts/python.exe -m pytest tests/corpus/test_gate_regression_detection.py tests/corpus/test_overmask_ledger_guard.py tests/test_gate_d.py tests/test_leak_v2.py tests/test_masking_correctness.py tests/test_precision_metric.py tests/test_corpus_no_crash.py tests/test_subset_iter_coverage.py tests/test_seam_boundary.py tests/test_by_entity.py
корпуса:        venv/Scripts/python.exe -m pytest tests/test_corpus_v2_axis_coverage.py tests/test_corpus_v2_reproducible.py tests/test_corpus_v2_structure_groups.py tests/test_corpus_v2_value_tricks.py tests/test_gold_type_contract.py tests/test_corpus_v2_manifest.py
...
1. **Пушить сразу после приёмки этапа** (запрет на пуш снят владельцем
   2026-08-17: месяц без пуша — работа пяти сессий осела на одной машине).
...
- Коммитить группами, чтобы можно было откатить точечно; пуш — по запрету 1.
