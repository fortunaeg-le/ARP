...
Что блокируется:
  1. `git push --force` / `--force-with-lease` / `-f` / `+refspec` — перезапись
     опубликованной истории. ОБЫЧНЫЙ `git push` разрешён с 2026-08-17 (решение
     владельца, CLAUDE.md запрет 1: пушить сразу после приёмки этапа).
...
#: Признак ПРИНУДИТЕЛЬНОЙ отправки. Обычный `git push` разрешён (CLAUDE.md,
#: запрет 1 в редакции 2026-08-17: пушить сразу после приёмки этапа), а вот
#: перезапись уже опубликованной истории — нет: она уничтожает чужие коммиты на
#: сервере молча и необратимо, и именно этим отличается от обычной отправки.
#: `--force-if-includes` включён: он смягчает `--force`, но остаётся force.
FORCE_FLAG = re.compile(
    r"""(^|[\s"'])(--force(-with-lease|-if-includes)?(=[^\s"']*)?|-f)($|[\s"'])""",
    re.IGNORECASE,
)
#: Вторая дорога к тому же: `git push origin +main` — плюс перед refspec это
#: force по определению git. Замок без неё закрывал бы парадную дверь при
#: открытой чёрной (урок DOG-REPO-BLOB: запрет по одному признаку не держит).
FORCE_REFSPEC = re.compile(r"""(^|[\s"'])\+[^\s"';|&]+""")
#: Где заканчивается КОМАНДА push: дальше уже другая команда, и её флаги к push
#: отношения не имеют (`git push origin main && rm -f x` — это не force-push).
...
def check_shell(raw_command: str, cwd: str = ""):
    ...
    m = PUSH.search(command)
    if m:
        # Хвост ИМЕННО этой команды: до первого разделителя. Иначе `-f` соседней
        # команды в цепочке читался бы как флаг push.
        tail = CMD_SEP.split(command[m.end():], maxsplit=1)[0]
        forced = FORCE_FLAG.search(tail) or FORCE_REFSPEC.search(tail)
        if forced:
            deny(
                "ЗАБЛОКИРОВАНО (замок force-push). Обычная отправка разрешена, но эта\n"
                "команда переписывает уже опубликованную историю:\n\n    %s\n\n"
                "Признак: %s. --force и --force-with-lease (и `+refspec`) уничтожают\n"
                "на сервере коммиты, которых нет локально, — молча и необратимо. У\n"
                "репозитория несколько рабочих машин, и работа целой сессии уже\n"
                "оседала не там, где её ждали.\n\n"
                "Нужно разойтись с сервером — сначала git fetch и разбор расхождения\n"
                "по существу; перезапись истории — отдельное решение владельца.\n"
                "См. CLAUDE.md, запрет 1.\n" % (command.strip(), forced.group(0).strip())
            )
