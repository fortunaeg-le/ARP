...
- `ValueError` (встроенное) — если расширение (в нижнем регистре) не входит в `{".docx", ".txt"}`. Сообщение: `f"Неподдерживаемый формат: {ext}. Поддерживаются: .docx, .txt"`.
...
- `"txt_line"`: ключи `line_index` (int, всегда присутствует) и `encoding` (str, `"utf-8-sig"` или `"cp1251"`, всегда присутствует).
...
`extract` не принимает предварительно распарсенных структур — только путь к файлу. Единственная проверка — существование файла (`Path(path).is_file()`) и расширение. Содержимое файла не валидируется заранее (например, битый .docx приведёт к исключению из `python-docx`, не перехватывается и не оборачивается).
...
- `.txt`-файлы, использующие кодировки, отличные от `utf-8-sig`/`cp1251`, приведут к необработанному `UnicodeDecodeError` (не входит в зафиксированные требования блока).
