...
_LOCAL_SEAM_ANCHORS = {
    ...
    # ЭТАП DEBTS (0c-B). У PASSPORT якорь тоже живёт ВНУТРИ паттерна, а до
    # закрытия 0c-B он лежал внутри СПАНА — и `layout_repair._strict_seam_ok`
    # видел его как часть значения («паспорт 08 19 096084» не «голая цифровая
    # цепь»). Как только спан сузился до значения, ремонт стыка стал требовать
    # якорь и не находил его нигде: PASSPORT в _SEAM_TYPES нет, поля `anchor:`
    # у типа нет. Зеркало головы паттерна возвращает ровно прежнее поведение
    # ремонта — и НЕ включает PASSPORT в сборку multispan (_SEAM_TYPES не
    # тронут): здесь только страж, а не новый путь детекции.
    # ЭТАП SEAM-JOIN: у PASSPORT ДВЕ формы значения и ДВА разных якоря. Зеркало
    # знало голову только первой («паспорт 62 11 029696»), и вторая — «код
    # подразделения 704-068» — отвергалась стражем сборки ВСЕГДА: собранное
    # «704-068» это голая цифровая цепь, а слова «паспорт» рядом с кодом
    # подразделения в документе нет. 57 отказов на 47 документах класса вёрстки
    # (разведка experiments/seam_join_seamlog.py) — крупнейший единичный
    # блокиратор класса. Зеркало головы ВТОРОГО паттерна entity_types.yaml.
    "PASSPORT": re.compile(
        r"(?i)(?:паспорт|серия|код\s*(?:подраздел[а-я]*|подр)\.?)"),
    # ЭТАП SEAM-JOIN: у PHONE якоря в конфиге нет вовсе (форма значения
    # доказывает тип сама), поэтому телефон, СОБРАННЫЙ через разрыв и потерявший
    # при сборке скобки/плюс, страж отвергал как голую цепь: «Тел.: 8-499-661-94-74».
    # Зеркало не даёт телефону новых прав на посегментной дороге — только на
    # сборке, где без якоря не принимается НИЧЕГО.
    "PHONE": re.compile(r"(?i)(?:тел|моб|факс|whatsapp|вайбер|viber)"),
}
...
def _match_seam_type(norm: str, chain, spec) -> str | None:
    ...
    ПОЧЕМУ СТРАЖ 2 ЗДЕСЬ ШЛАГБАУМ, ХОТЯ НА ПОСЕГМЕНТНОЙ ДОРОГЕ КС — ТОЛЬКО
    СИГНАЛ. Это не рассогласование с инвариантом этапа 4
    (regex_detector.detect_regex, «ЭТАП 4 — КС как сигнал уверенности, а не
    шлагбаум»), а РАЗНЫЕ ПРАВИЛА ДЛЯ РАЗНЫХ ДОРОГ — решение владельца, этап
    CROSSTYPE, docs/STATE.md §6. Читать так:
      * посегментная детекция: значение лежит в тексте ЦЕЛИКОМ, паттерн его
        нашёл, якорь назвал. КС — сигнал: опечатка в реквизите не делает его
        не-реквизитом. Цена ошибки — лишняя маска, порча документа;
      * СБОРКА (здесь): значения в тексте НЕТ, оно КОНСТРУИРУЕТСЯ из кусков по
        разные стороны разрыва. КС — последняя проверка, что собран реквизит, а
        не совпадение длин. Цена ошибки другая и хуже: не пропуск, а неверная
        маска и неверное ВОССТАНОВЛЕНИЕ — склеенный из чужих кусков «реквизит»
        вернётся в документ не тем, чем был.
    Отрицательный контракт стража живёт в
    tests/test_boundary_entities.py::TestSeamAssemblyGuards::
    test_broken_checksum_is_not_stitched. Снятие КС отсюда ЗАМЕРЕНО и ОТКАЗАНО:
    оно закрывает 4 из 6 случаев линии «з» и роняет утечку на 15 сущностей на
    корпусе v1, но на реальном документе не даёт ничего (там КС сходится и
    сборка работает без правки), потому что синтетический корпус валидных
    контрольных сумм не порождает вовсе. Долг — DEFGATE-SEAM-CHECKSUM-TRADEOFF
    в docs/FINDINGS.md; ослаблять страж ради его цифры нельзя."""
