...
def detokenize_file(
    ...
    if ext not in _SUPPORTED:
        if ext == ".pdf":
            # PDF-ARCH: восстановление в принесённый .pdf с сохранением
            # оформления НЕ входит в блок (нет rewriter-адаптера, как для
            # docx/xlsx/pptx) — но это не тупик, а другой, уже рабочий путь:
            # decrypt (текстовый) формат-независим и восстанавливает ответ
            # LLM как обычный текст.
            raise ValueError(
                "Восстановление файлов .pdf через decrypt-file не поддерживается "
                "(в PDF нет структуры для вставки исходных значений на место "
                "токенов с сохранением оформления, как у .docx/.xlsx/.pptx). "
                "Используйте команду decrypt: скопируйте ответ LLM (текст с "
                "токенами [TYPE_N]) и передайте его на stdin — decrypt восстановит "
                "исходные значения в самом тексте."
            )
        raise ValueError(
