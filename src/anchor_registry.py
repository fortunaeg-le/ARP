...
class OrgAnchorDetector(AnchorDetector):
    ...
    # МЕХАНИЗМ 2 (C′) — КОНСТРУКЦИЯ ОПРЕДЕЛЕНИЯ ТЕРМИНА как АНТИ-ЯКОРЬ. В разделе
    # определений договора термин ВВОДИТСЯ конструкцией «"X" означает …» / «"X" — это …»
    # / «под "X" понимается …» / «"X" имеет значение …» — синтаксически неотличимо от
    # представления фирмы, но это ОПРЕДЕЛЕНИЕ, не сущность. Кандидат из такой конструкции
    # в реестр ORG не регистрируется. Признак СТРУКТУРНЫЙ (форма конструкции), НЕ список
    # слов: ни «Сторона», ни «Общество» нигде не перечислены. «означа\w+» покрывает
    # «означает/означают»; «понима\w+» — «понимается/понимают» (в т.ч. «под «X»
    # понимается»); тире перед «это» отсекает голое «X это Y», чтобы не задеть прозу.
    _DEF_TAIL_RE = re.compile(
        r"(?i)^[\s,)]*(?:означа\w+|[—–\-]\s*это\b|имеет\s+значени\w+|понима\w+)"
    )
    _DEF_UNDER_RE = re.compile(r"(?i)под\s*$")   # «под "X" понимается»

    @staticmethod
    def _is_collective_head(name):
        """True, если ВЕРШИНА (первое слово) бескавычного имени — МНОЖЕСТВЕННОЕ
        нарицательное (роль-коллектив «Акционеры»/«Стороны»/«Участники»), а не
        собственное имя. Разбирает первый правдоподобный (score>=0.1) вариант
        pymorphy: собственное (Name/Surn/Patr/Geox/Orgn/Trad) — оставляем (False);
        plur+NOUN — коллектив (True). Компания сингулярна: «Сбербанк»/«Восход»/
        «Ромашка» — sing, проходят."""
        ws = _WORD_RE.findall(name)
        if not ws:
            return False
        for p in _MORPH(ws[0]):
            if p.score < 0.1:
                continue
            t = str(p.tag)
            if any(x in t for x in ("Name", "Surn", "Patr", "Geox", "Orgn", "Trad")):
                return False
            return ("plur" in t) and ("NOUN" in t)
        return False

    def _is_definition_term(self, low, oq, cq):
        """True, если ядро в кавычках [oq..cq] стоит в конструкции ОПРЕДЕЛЕНИЯ ТЕРМИНА
        (Mechanism 2). Проверяет хвост сразу за закрывающей кавычкой и, отдельно,
        связку «под «X» понимается» (маркер слева + «понима…» справа)."""
        tail = low[cq:cq + 30]
        if self._DEF_TAIL_RE.match(tail):
            return True
        if self._DEF_UNDER_RE.search(low[max(0, oq - 6):oq]) and \
                re.search(r"(?i)понима\w+", tail):
            return True
        return False
    ...
    def detect(self, seg, norm, low, omap, requisites) -> list:
        ...
        for (oq, cs, ce, cq) in _find_quoted(norm):
            ...
            if key is None:
                continue
            # МЕХАНИЗМ 2: «"X" означает …» — это ОПРЕДЕЛЕНИЕ ТЕРМИНА, не представление
            # сущности. Не регистрируем даже при org-форме рядом (реальная фирма почти
            # никогда не выступает подлежащим «означает/понимается/имеет значение»; в
            # корпусе таких ORG-конструкций 0 — проверено гейтом).
            if self._is_definition_term(low, oq, cq):
                continue
        ...
        for m in _ORGFORM_ANY_RE.finditer(low):
            ...
            if self._ROLE_BEFORE_FORM_RE.search(low[:m.start()]):
                continue
            # МЕХАНИЗМ 1 (C′): РАЗВЁРНУТАЯ форма-СЛОВО («Общество»/«Акционерное
            # общество»/«Товарищество»/«ответственность…») голое имя за собой БОЛЬШЕ
            # НЕ якорит. В договорах эти слова — ГЕНЕРИК/РОЛЬ жанра («Общество»
            # вводится как «именуемое в дальнейшем «Общество»», «ответственность» —
            # часть прозы), а не название: «Общество Заказчик»/«ответственность За
            # Неисполнение» уходили в реестр ORG, и проход 2 тиражировал их по всем
            # падежам (реальный договор → «решето»). Развёрнутая форма регистрирует ORG
            # только В ПАРЕ с кавычками (кавычная ветвь выше, форма примыкает к «Имя»)
            # или с ИНН/ОГРН юрлица (ветвь ниже). Аббревиатура (ООО/ПАО/АО/…) —
            # однозначный маркер юрлица, за ней голое имя оставляем («ПАО Сбербанк»).
            # На корпусе демоушен теряет 0 gold-ORG (0 бескавычных развёрнутых форм) и
            # снимает 4 ложных ORG-якоря — проверено гейтом D.
            ...
            if run is None:
                continue
            rs, re_ = run
            # МЕХАНИЗМ 1b (C′): организация — СИНГУЛЯРНАЯ именованная сущность. Голое
            # бескавычное имя за формой, чья ВЕРШИНА — МНОЖЕСТВЕННОЕ нарицательное
            # («Акционеры»/«Стороны»/«Участники»), это КОЛЛЕКТИВ-РОЛЬ, а не название.
            # Классический промах реального договора: «…Закона об АО Акционеры
            # договорились…» — «АО» здесь ссылка на закон (объект предлога «об»), а
            # «Акционеры» начинают новую клаузу; пАттерн B грёб их в ORG, и проход 2
            # тиражировал роль по ВСЕМУ документу (383 маски). Признак СТРУКТУРНЫЙ
            # (помета pymorphy plur+NOUN у вершины, без Name/Surn/Geox/Orgn/Trad —
            # собственные имена оставляем), НЕ список слов. На корпусе демоушен теряет
            # 0 gold-ORG (0 бескавычных множественных вершин) — проверено.
            if self._is_collective_head(norm[rs:re_]):
                continue
