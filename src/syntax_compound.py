...
docs/archive/reports/DETECTION_REBUILD.md.
...
def _appos_links(tokens, org_span: tuple[int, int], per_span: tuple[int, int]) -> bool:
    ...
    Эмпирика (docs/archive/reports/WAVE2_VERIFICATION.md, разбор в голой строке ячейки):
    ...
    for seg_id, ents in by_seg.items():
        ...
        for (os_, oe, oent, p) in candidate_pairs:
            ...
            merged.append(Entity(
                ...
                # Поглощённое ФИО не исчезает бесследно: составная ORG несёт его
                # в `shadowed`. Пока ORG в наборе пользователя — маска одна и
                # прежняя (смысловая целостность, ради которой склейка и
                # заведена). Как только ORG выключен — а в наборе по умолчанию
                # он выключен, — имя человека обязано получить СВОЮ маску, а не
                # уйти в LLM открытым текстом вместе со всей склейкой:
                # «ИП Беляев Олег Олегович» это персональные данные независимо
                # от того, что юрлицо оформлено. Восстановление — в
                # tokenizer._recover_shadowed.
                shadowed=[p],
            ))
            if oent is not None:
