...
# времени детекции, см. docs/archive/reports/RECON_REPORT.md) и всё равно ронял хвост адреса
...
# ЭТАП FIX-3 — ЗАВИСИМЫЕ МАРКЕРЫ ПОМЕЩЕНИЯ (долг NODES-FOOTER-PAGENUM).
# «стр.» в русском документе значит И «строение», И «страница». Как маркер
# помещения он самодостаточен не был никогда: строение — УТОЧНЕНИЕ дома, оно
# физически не может открывать адрес («г. Москва, ул. Мира, д. 5, стр. 2» — но
# никогда «стр. 2» само по себе). Нумерация же страницы стоит в колонтитуле
# ОДНА, без улицы и города, — и до этого этапа проходила якорь «помещение +
# число» («Стр. 1 из 3» -> [ADDRESS_1] из 3).
# Дефект ПРЕД-СУЩЕСТВУЮЩИЙ (воспроизводится на голом .txt), но этап NODES поднял
# его частоту до «каждая страница каждого документа»: колонтитулы стали
# читаться, а нумерация живёт именно там.
# Ограничитель СТРУКТУРНЫЙ, а не список запрещённых слов: зависимый маркер
# анкорит адрес ТОЛЬКО в присутствии улицы или локальности. Направление ошибки
# безопасное — настоящее строение без улицы и города в договоре не пишут.
_ADDR_DEPENDENT_PREMISE_MK = frozenset("стр строен строение".split())
...
def _premise_dependent_only(toks, cats) -> bool:
    """ЭТАП FIX-3: маркеры помещения в спане есть, но ВСЕ они зависимые («стр.»),
    а улицы и локальности рядом нет — значит это не адрес, а нумерация страницы.

    Проверяется именно СОСТАВ маркеров, а не текст вокруг: спан с «ул.»/«г.»
    рядом остаётся адресом со «стр.» внутри («ул. Мира, д. 5, стр. 2»), потому
    что у него есть независимый якорь."""
    prem = [(w, s) for (s, _, w, c, _, _) in toks if c == "premise"]
    if not prem or not all(w.lower() in _ADDR_DEPENDENT_PREMISE_MK
                           for w, _ in prem):
        return False
    if cats & {"street", "locality"}:
        return False
    # ОГРАНИЧИТЕЛЬ СУЖЕН ПО ЗАМЕРУ (полный гейт после первой редакции показал
    # цену: 2 потерянных адреса, долг 43->45). Потерянное — ХВОСТ адреса,
    # отрезанный переносом строки: «…33 км автодороги М-3 «Дон»\n, стр. 3».
    # Различие с нумерацией страницы устойчиво и видно в самом тексте:
    # нумерация — «Стр.» С ЗАГЛАВНОЙ И В НАЧАЛЕ спана (она открывает
    # колонтитул), адресный хвост — строчное «стр.» после запятой. Строчный
    # маркер-одиночка и до этапа не маскировался (проверено прогоном до
    # правки: «стр. 12» -> чисто), поэтому сужение не возвращает старый дефект,
    # а только перестаёт задевать живое.
    first_w, first_s = prem[0]
    if not (first_w[:1].isupper() and first_s == toks[0][0]):
        return False
    # СПУТНИКОМ считается и ГОЛЫЙ ТОПОНИМ: «п. Развилка, Промзона, стр. 4» —
    # настоящий адрес без слова «улица» (жёсткий контракт GOLDEN, он же и
    # поймал первую, слишком узкую редакцию этого ограничителя). Сам зависимый
    # маркер из счёта топонимов исключён — иначе Titlecase-«Стр.» подтверждал
    # бы сам себя.
    for (_, _, w, c, is_num, cap) in toks:
        if cap and not is_num and w.lower() not in _ADDR_DEPENDENT_PREMISE_MK:
            return False
    return True
...
def _addr_span_strong(text: str, s: int, e: int) -> bool:
    ...
    if "street" in cats and (has_cap or has_num):
        return True
    if "premise" in cats and has_num and not _premise_dependent_only(toks, cats):
        return True
...
def _recover_address_over_weak_per(
    ...
    PERSON — новое правило там же, docs/archive/reports/T_ARB_REPORT.md). Многословный
